// // ______----- the NO VOTE JUST FTECH MONGO ____________
...
//         // Inject dummy votes = 0 if not present
//         const enriched = data.map((c) => ({
//           ...c,
//           votes: c.votes || 0,
//         }));

//         const total = enriched.reduce((sum, c) => sum + c.votes, 0);
...
//       (c) => `${c.candidate_name} (#${c.candidate_number})`
//     ),
//     datasets: [
//       {
//         label: "Votes",
...
//                   <img
//                     src={c.candidate_image || "https://via.placeholder.com/150"}
...
const ElectionResult = () => {
  ...
  const fetchRejectedVotes = async () => {
    try {
      ...
      // Make sure the correct key exists
      if (
        res.data &&
        (typeof res.data.rejectedCount === "number" ||
          typeof res.data.rejectedCount === "string")
      ) {
        setRejectedCount(Number(res.data.rejectedCount));
      } else {
